...
export type ValidError = Record<string, { errors: string[]; value?: FormDataEntryValue | null }>;
...
export const validateAsync = async <T extends z.ZodObject>(
  ...
  const obj =
    formDataOrObj instanceof FormData ? Object.fromEntries(formDataOrObj.entries()) : formDataOrObj;
  try {
...
export const comparePassword = (plainPasswd: string | undefined, encryptPasswd: string) =>
  compare(plainPasswd || "", encryptPasswd);

export const existsFile = (filePath: string | undefined | null) => {
  // console.log("fffffff>>", filePath);
